...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif spellId == 10059 then --Штормград
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, stormwind), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, stormwind), "PARTY")
			end
		end
	elseif spellId == 11416 then --Стальгорн
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, ironforge), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, ironforge), "PARTY")
			end
		end
	elseif spellId == 11419 then --Дарнас
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, darnassus), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, darnassus), "PARTY")
			end
		end
	elseif spellId == 32266 then --Экзодар
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, exodar), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, exodar), "PARTY")
			end
		end
	elseif spellId == 49360 then --Терамор
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, theramore), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, theramore), "PARTY")
			end
		end
	elseif spellId == 11417 then --Оргриммар
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, orgrimmar), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, orgrimmar), "PARTY")
			end
		end
	elseif spellId == 11418 then --Подгород
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, undercity), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, undercity), "PARTY")
			end
		end
	elseif spellId == 11420 then --Громовой утес
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, thunderBluff), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, thunderBluff), "PARTY")
			end
		end
	elseif spellId == 32267 then --Луносвет
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, silvermoon), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, silvermoon), "PARTY")
			end
		end
	elseif spellId == 49361 then --Каменор
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, stonard), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, stonard), "PARTY")
			end
		end
	elseif spellId == 33691 then --Шаттрат
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, shattrath), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, shattrath), "PARTY")
			end
		end
	elseif spellId == 53142 then --Даларан1
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, dalaran1), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, dalaran1), "PARTY")
			end
		end
	elseif spellId == 88345 then --Тол Барад (альянс)
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, tolBarad1), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, tolBarad1), "PARTY")
			end
		end
	elseif spellId == 88346 then --Тол Барад (орда)
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, tolBarad2), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, tolBarad2), "PARTY")
			end
		end
	elseif spellId == 132620 then --Вечноцветущий дол (альянс)
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, valeEternal1), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, valeEternal1), "PARTY")
			end
		end
	elseif spellId == 132626 then --Вечноцветущий дол (орда)
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, valeEternal2), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, valeEternal2), "PARTY")
			end
		end
	elseif spellId == 176246 then --Преграда Ветров (альянс)
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, stormshield), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, stormshield), "PARTY")
			end
		end
	elseif spellId == 176244 then --Копье Войны (орда)
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, warspear), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.PortalYell:format(args.sourceName, warspear), "PARTY")
			end
		end
	elseif spellId == 224871 then --Даларан2
		if self.Options.YellOnPortal then
			if IsInRaid() then
				SendChatMessage(L.PortalYell:format(args.sourceName, dalaran2), "RAID")
			elseif IsInGroup() then
	...
	elseif spellId == 29893 and self:AntiSpam(2.5, 1) then --Источник душ
		if self.Options.YellOnSoulwell then
			...
			elseif IsInGroup() then
				SendChatMessage(L.SoulwellYell:format(args.sourceName, soulwell), "PARTY")
			end
		end
	end
end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	if spellId == 20707 then --Камень души
		...
		if self.Options.YellOnSoulstone then
			...
			elseif IsInGroup() then
				SendChatMessage(L.SoulstoneYell:format(args.sourceName, soulstone, args.destName), "PARTY")
			end
		end
	end
end
...
function mod:SPELL_CREATE(args)
	...
	elseif spellId == 201351 and self:AntiSpam(2, 1) then --Обильное угощение
		if self.Options.YellOnLavish then
			...
			elseif IsInGroup() then
				SendChatMessage(L.SoulwellYell:format(args.sourceName, hearty), "PARTY")
			end
		end
	end
end
...
function mod:SPELL_RESURRECT(args)
	...
	if spellId == 95750 then --Воскрешение камнем души
		if self.Options.YellOnResurrect then
			if IsInRaid() then
				SendChatMessage(L.SoulstoneYell:format(args.sourceName, rebirth, args.destName), "RAID")
			elseif IsInGroup() then
				SendChatMessage(L.SoulstoneYell:format(args.sourceName, rebirth, args.destName), "PARTY")
			end
		end
